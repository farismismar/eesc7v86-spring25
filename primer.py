...
def create_channel(N_sc, N_r, N_t, shadow_fading_margin_dB=8, channel='rayleigh'):
    ...
    if shadow_fading_margin_dB is None:
        shadow_fading_margin_dB = 0
        
    if channel == 'awgn':
        return _create_awgn_channel(N_sc, N_r, N_t)
    
    if channel == 'ricean':
        return _create_ricean_channel(N_sc, N_r, N_t, K_factor=K_factor, sigma_dB=shadow_fading_margin_dB)

    if channel == 'rayleigh':
        return _create_ricean_channel(N_sc, N_r, N_t, K_factor=0, sigma_dB=shadow_fading_margin_dB)

    if channel == 'CDL-C':
        return _generate_cdl_c_channel(N_sc, N_r, N_t, sigma_dB=shadow_fading_margin_dB)

    if channel == 'CDL-E':
        return _generate_cdl_e_channel(N_sc, N_r, N_t, sigma_dB=shadow_fading_margin_dB)
...
def _create_ricean_channel(N_sc, N_r, N_t, K_factor, sigma_dB):
    global np_random
...
def _generate_cdl_c_channel(N_sc, N_r, N_t, sigma_dB):
    global np_random
    ...
    for tap in range(num_taps):
        ...
        H_tap = np.sqrt(powers_linear[tap] * shadow_fading[:, :, None]) * \
...
def _generate_cdl_e_channel(N_sc, N_r, N_t, sigma_dB):
    global np_random
...
noise_figure = 7                         # Receiver noise figure [dB]
...
MIMO_estimation = 'LMMSE'                   # Also: perfect, LS, LMMSE
...
K_factor = 2                             # For Ricean
shadowing_std_dev = 4                    # in dB

# Transmit SNR in dB
transmit_SNR_dB = [-10, -5, 0, 5, 10, 15, 20, 25, 30]
...
df_detailed = pd.DataFrame(columns=['n', 'snr_dB', 'Tx_EbN0_dB', 'dist', 'Tx_Pwr_dBm',
                                   'channel_estimation_error', 'PL_dB', 'Rx_Pwr_dBm', 'sinr_receiver_before_eq_dB', 'BER', 
                                   'sinr_receiver_after_eq_dB', 'Rx_EbN0_dB'])
df_bler = pd.DataFrame(columns=['snr_dB', 'Tx_EbN0_dB', 'BLER'])
...
for item, snr_dB in enumerate(transmit_SNR_dB):
    ...
    for n_transmission in range(max_transmissions):
        ...
        if n_transmission % 10 == 0:
            print()
            print(' | '.join(df_detailed.columns))
            _print_divider(rep=100)

        # Channel coherence time is equal to one transmission
        H = create_channel(N_sc, N_r, N_t, channel=channel_type, shadow_fading_margin_dB=None)
        
        # The coordinates of a user
        x_coord = np_random.uniform(-500, 500)
        y_coord = np_random.uniform(-500, 500)
        
        d = np.sqrt(x_coord ** 2 + y_coord ** 2)
        
        # Add large scale gain
        G = compute_large_scale_fading(d, f_c, D_t_dB=18, D_r_dB=-1, pl_exp=2)
        G_fading = _dB(G) + np_random.normal(loc=0, scale=np.sqrt(shadowing_std_dev))  # Add shadowing \chi_\sigma
        G_fading = _linear(G_fading)
        
        # Note to self: G_fading can create detection problems for H
        ...
        W = equalize_channel(H_est, snr_dB, algorithm=MIMO_equalization)# / np.sqrt(G_fading)
        
        # W@H_est is ones or I
        z = _matrix_vector_multiplication(W, Y)
        v = _matrix_vector_multiplication(W, noise)

        P_x_hat_dB = _dB(np.linalg.norm(W[0,:,:], ord='fro') ** 2) + PL_dB + _dB(P_X)
        P_v_dB = _dB(P_noise) + _dB(np.linalg.norm(W[0,:,:], ord='fro') ** 2)
        ...
        to_append_i = [n_transmission, snr_dB, EbN0_dB, d, P_X_dBm,
                       estimation_error, PL_dB, P_Hx_dBm, snr_rx_dB_pre_eq, BER_i, snr_rx_dB, EbN0_rx_dB]
        df_to_append_i = pd.DataFrame([to_append_i], columns=df_detailed.columns).round(4)
        
        print(' | '.join(map(str, df_to_append_i.iloc[0, :].to_list())))
    ...
    to_append = [snr_dB, EbN0_dB, BLER]
    df_to_append = pd.DataFrame([to_append], columns=df_bler.columns)
    
    if df_bler.shape[0] == 0:
        df_bler = df_to_append.copy()
    else:
        df_bler = pd.concat([df_bler, df_to_append], ignore_index=True, axis=0)

end_time = time.time()
print(f'Time elapsed: {((end_time - start_time) / 60.):.2f} mins.')

df_detailed.to_csv('simulation_data.csv')
df_bler.to_csv('simulation_bler_data.csv')

plot_performance(df_detailed, 'Tx_EbN0_dB', 'BER', semilogy=True, filename=None)
plot_performance(df_bler, 'Tx_EbN0_dB', 'BLER', semilogy=True, filename=None)
