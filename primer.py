...
P_TX = 1                                 # Signal transmit power [W]
...
H = create_channel(N_sc, N_r, N_t, channel=channel_type, shadow_fading_margin_dB=0)
...
for item, snr_dB in enumerate(transmit_SNR_dB):
    ...
    for n_transmission in range(max_transmissions):
        ...
        G = compute_large_scale_fading(d, f_c, D_t_dB=18, D_r_dB=-1, pl_exp=2.3)
        ...
        snr_rx_dB_pre_eq = _dB(P_Hx) - _dB(P_noise + P_interference) - noise_figure
...
plot_performance(df, 'Tx_EbN0_dB', 'BLER', semilogy=True, filename=None)
plot_scatter(df_detailed, 'distance', 'PL_dB', filename=None)
